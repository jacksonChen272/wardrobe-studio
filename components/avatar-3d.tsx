...
export function Avatar3D({ gender, angle, zoom, garments }: { gender: 'female' | 'male'; angle: number; zoom: number; garments: Garment[] }) {
  ...
  useEffect(() => {
    ...
    const skin = new THREE.MeshPhysicalMaterial({ color: gender === 'female' ? 0xd9aa8f : 0xc89476, roughness: .62, metalness: 0, clearcoat: .08 });
    ...
    const hair = new THREE.Mesh(new THREE.SphereGeometry(1, 40, 24, 0, Math.PI*2, 0, Math.PI*.58), new THREE.MeshStandardMaterial({ color: gender === 'female' ? 0x241b17 : 0x34251d, roughness:.9 })); hair.position.set(0,2.39,-.03); hair.scale.set(.44,.5,.45); avatar.add(hair);
    const featureMaterial = new THREE.MeshStandardMaterial({ color:0x392b27, roughness:.8 });
    [-.14,.14].forEach((x) => { const eye=new THREE.Mesh(new THREE.SphereGeometry(.035,16,10),featureMaterial); eye.position.set(x,2.32,.405); avatar.add(eye); });
    const nose=add(new THREE.ConeGeometry(.055,.16,16),[0,2.21,.44],[1,1,1],[Math.PI/2,0,0]); nose.material=skin;
    ...
    [...garments].sort((a,b) => a.z-b.z).forEach((item, index) => {
      ...
      const wrapsBody = item.category === '上衣' || item.category === '下身';
      const geometry = wrapsBody
        ? new THREE.CylinderGeometry(item.category === '上衣' ? .69*shoulder : .67*hip, item.category === '上衣' ? .55 : .48, fit.h, 40, 4, true, -Math.PI/2, Math.PI)
        : new THREE.PlaneGeometry(fit.w, fit.h, 18, 18);
      const material = new THREE.MeshStandardMaterial({ map:texture, transparent:true, side:THREE.DoubleSide, depthWrite:false, alphaTest:.04, roughness:.78, metalness:0 });
      const mesh = new THREE.Mesh(geometry, material); mesh.position.set(0, fit.y, wrapsBody ? 0 : fit.z + index*.012); if (wrapsBody) mesh.scale.x=1.06; mesh.scale.multiplyScalar(Math.max(.45, item.scale)); mesh.rotation.z = THREE.MathUtils.degToRad(item.rotation); mesh.renderOrder = 10 + index; avatar.add(mesh);
    });
